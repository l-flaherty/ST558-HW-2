...
####still need to fix knitr in pt 2, and summary in pt 4###
...
names(pre_bp)=paste0("Subject_", 1:length(pre_bp))   #add names attribute#
names(post_bp)=paste0("Subject_", 1:length(post_bp)) #add names attribute#
...
placebo=data.frame(
  ...
  pre_bp,
  post_bp,
  diff_bp=post_bp-pre_bp,                              #add difference variable directly to df#
  low_bp=ifelse(post_bp<120, TRUE, FALSE))             #add variable for low bp after treatment#
...
mylist[-2]                                             #even more ways to access df in list#

mylist[["placebo"]]["pre_bp"]                          #access specific part of df from list#
...
treatment$status=character(nrow(treatment))            #initialize#
placebo$status=character(nrow(placebo))                #initialize#

for(i in 1:nrow(mylist[["treatment"]])) {              #just to practice 'for'#  
  a=mylist[["treatment"]][["post_bp"]]                 #to make less cluttered#
  if(a[i]>130) {                                       #note if data bigger, wouldn't...#
    status[i]=="high"                                  #...want to keep calling object...#
  } else if (a[i]>120) {                               #for performance reasons#
    status[i]=="borderline"
  } else {
    status[i]=="optimal"
  }
...
for(i in 1:nrow(mylist[["placebo"]])) {                #just to practice 'for'#
  a=mylist[["placebo"]][["post_bp_placebo"]]           #to make less cluttered#
  if(a[i]>130) {      
    status[i]=="high"
  } else if (a[i]>120) {
    status[i]=="borderline"
...
#####5. Function Writing#####
myfunction=function(l, func=mean) {       #set default function to mean#
  df1=l[[1]]; df2=l[[2]]
  a=c("pre_bp", "post_bp", "diff_bp")     #variables to select, assume shared names#
  b=vector()                              #initialize#
  
  for (i in 1:length(a)) {                #apply function (no l/sappy yet)#
    b[(2*i)-1]=func(df1[,a[i]])
    b[(2*i)]=func(df2[,a[i]])
  }
